...
// merges with existing catalog (keeps cached tools for servers that didn't
// respond), and saves to disk cache.
func (c *Catalog) RefreshAll(ctx context.Context) error {
	...
	for _, t := range c.tools {
		if _, responded := serverTools[t.ServerName]; !responded {
			all = append(all, t)
		}
	}
	c.mu.RUnlock()
