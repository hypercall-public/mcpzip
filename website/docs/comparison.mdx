...
title: Comparison
description: How mcpzip compares to raw MCP, other proxies, and alternative approaches
---
...
<ComparisonTable
  ...
  rows={[
    ...
    ["OAuth support", "Per-client", "Centralized + token reuse"],
...
- You have 1-2 servers with < 20 tools total
- You need the absolute simplest setup
- You don't care about context overhead
...
mcpzip adds one extra roundtrip per tool use (the search step). Here's what you give up and what you gain:
...
The search step adds ~200-500ms with LLM search, or <1ms with keyword search only. Compared to the latency savings from reduced context (fewer tokens to process per message), most users see a **net speed improvement**.
:::
