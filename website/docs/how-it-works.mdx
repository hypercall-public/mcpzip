...
Here's what happens when Claude wants to send a Slack message:
...
sequenceDiagram
    ...
    activate M
    M-->>C: Full JSON schema {channel, text, ...}
    deactivate M
...
mcpzip uses a two-tier search system. Here's how it decides which path to take:

```mermaid
flowchart TD
    Q[Query arrives at search_tools] --> C{Gemini API key<br/>configured?}
    ...
    KW --> SCORE[Score & Rank Results]
    KW2 --> MERGE[Merge & Deduplicate]
    LLM --> MERGE
...
A typical tool schema consumes **300-500 tokens**. With 10 servers averaging 25 tools each, that's **75,000-125,000 tokens** loaded on every message -- before the conversation even starts.
...
1. **Context window saturation**: With 250 tools at ~350 tokens each, you burn 87,500 tokens before the conversation starts. On Claude with 200K context, that's 44% of your context gone.
...
1. **Parses** the prefixed name (`slack__send_message` -> server: `slack`, tool: `send_message`)
...
If the connection was idle, it's automatically re-established. If the server is unreachable, a clear error is returned.
