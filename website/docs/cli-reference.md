...
curl -sSL https://github.com/hypercall-public/mcpzip/releases/latest/download/mcpzip-$(uname -s)-$(uname -m) -o mcpzip
chmod +x mcpzip
sudo mv mcpzip /usr/local/bin/
```
...
mcpzip serves from its disk cache on startup, so it's available immediately. The background refresh connects to upstream servers concurrently and merges any new or changed tools into the catalog. If a server fails to connect, its cached tools are preserved.
:::
...
:::tip
If you already have MCP servers configured in Claude Code, use `mcpzip migrate` instead -- it's faster and preserves your existing setup.
:::
...
```mermaid
flowchart LR
    A[Read Claude Code<br/>config] --> B[Extract all<br/>mcpServers]
    B --> C[Write mcpzip<br/>config.json]
    B --> D[Replace Claude Code<br/>servers with single<br/>mcpzip entry]

    style A fill:#1a1a2e,stroke:#60A5FA,color:#fff
    style B fill:#1a1a2e,stroke:#A78BFA,color:#fff
    style C fill:#1a1a2e,stroke:#5CF53D,color:#fff
    style D fill:#1a1a2e,stroke:#5CF53D,color:#fff
```
